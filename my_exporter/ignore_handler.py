...
def load_ignore_patterns(ignore_file: str = '.gitignore') -> PathSpec:
    ...
    - ignore_file (str, optional):
        Path to the ignore file. Defaults to '.gitignore'.

    Returns:
    - PathSpec:
        Compiled path specification for ignore patterns.

    Raises:
    - FileNotFoundError:
        If the specified ignore file does not exist.
    - IOError:
        If an I/O error occurs while reading the ignore file.
    """
...
def load_include_patterns(include_file: str) -> PathSpec:
    ...
    Returns:
    - PathSpec:
        Compiled path specification for include patterns.

    Raises:
    - FileNotFoundError:
        If the specified include file does not exist.
    - IOError:
        If an I/O error occurs while reading the include file.
    """
