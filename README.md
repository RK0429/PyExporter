...
You can install `pyexporter` via [PyPI](https://pypi.org/) using `pip`:

```bash
pip install pyexporter
```

Alternatively, you can install it directly from the source:

```bash
git clone https://github.com/RK0429/pyexporter.git
cd pyexporter
pip install .
```
...
After installation, you can use the `pyexporter` CLI tool to export your folder contents.

**Basic Usage:**

```bash
pyexporter --root-dir path/to/project --output-file exported.txt
```
...
```bash
pyexporter --root-dir ./my_project --output-file project_contents.txt
```

### Programmatic Usage

You can also use `pyexporter` as a library within your Python projects.
